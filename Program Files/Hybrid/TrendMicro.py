...
class Trend_Micro_Scraper:
    """Trend_Micro Scraper Class"""

    url = "https://success.trendmicro.com/dcx/s/solution/1105727-end-of-life-trend-micro-products-versions?language=en_US&sfdcIFrameOrigin=null"
    ...
    def eol_data_generator(self, url, output_filename):
        ...
        df["End-of-Life"] = df["End-of-Life"].apply(lambda x: self.convert_and_fill_date(x))
        df.drop(df[df['End-of-Life'] == 'See note'].index, inplace=True)
        df['Source URL'] = url
        df['Vendor'] = 'Trend Micro'
...
scraper = Trend_Micro_Scraper()
scraper.eol_data_generator(scraper.url, "CSV\Hybrid\TrendMicro.csv")